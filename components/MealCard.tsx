...
export default function MealCard({ meal, currentUserId, onReact, onDelete, onOpenComments }: {
  meal: Meal
  currentUserId: string
  onReact: (mealId: string, emoji: string) => void
  onDelete?: (mealId: string) => void
  onOpenComments: (meal: Meal) => void
}) {
  ...
  return (
    <Fragment>
      <div className="meal-card">
        ...
        {/* meal content — clicking this opens comments */}
        <div onClick={() => onOpenComments(meal)} style={{ cursor: 'pointer' }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 12, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <button
            onClick={() => onOpenComments(meal)}
            style={{
              marginLeft: 'auto', background: 'none', border: 'none',
              fontSize: 13, color: 'var(--muted)', cursor: 'pointer', padding: 0,
            }}
          >
            💬 comment
          </button>
